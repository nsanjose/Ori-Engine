...
void Mesh::CreateBuffers(Vertex * vertices, int numVerts, unsigned int * indices, int numInds, ID3D11Device * device)
{
	...
	device->CreateBuffer(&vbd, &initialVertexData, &mcp_vertex_buffer);
	...
	ibd.ByteWidth = sizeof(unsigned int) * numInds;         // 3 = number of indices in the buffer
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER; // Tells DirectX this is an index buffer
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	ibd.StructureByteStride = 0;

	m_index_count = numInds; // Assign index count
	m_vertex_count = numVerts; // Assign vertex count
	...
	// Actually create the buffer with the initial data
	// - Once we do this, we'll NEVER CHANGE THE BUFFER AGAIN
	device->CreateBuffer(&ibd, &initialIndexData, &mcp_index_buffer);
}
